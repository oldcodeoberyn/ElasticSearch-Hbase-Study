...
public class EsController
{
        ...
        @RequestParam( value = "pageSize", defaultValue = "3" ) Integer pageSize )
    {
        try
        {
            keyWords = new String( keyWords.getBytes( "UTF-8" ), "UTF-8" );
        }
    ...
    public String detailArticleById( @PathVariable( value = "id" ) String id, Model modelMap ) throws IOException
    {
        ...
        doc.setAuthor( new String( doc.getAuthor().getBytes(  ), "UTF-8" ) );
        doc.setTitle( new String( doc.getTitle().getBytes(  ), "UTF-8" ) );
        doc.setContent( new String( doc.getContent().getBytes(  ), "UTF-8" ) );
        doc.setDescribe( new String( doc.getDescribe().getBytes(  ), "UTF-8" ) );
